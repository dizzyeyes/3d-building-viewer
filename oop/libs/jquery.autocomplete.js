...
(function($){
    $.fn.extend({
        'AutoCompleteMe': function(option){
            return this.each(function(){
                if (!(this && this.tagName === 'INPUT' && this.type === 'text')) return;

                if(this.ControllerMe)
                    this.ControllerMe.setOption(option);
                else {
                    if($.isPlainObject(option))
                        this.ControllerMe = new ControllerMe(this, option);
                }
            });
        }
    });
    ...
    var ControllerMe = function(input, option){
    ...
    var _setupSearchView = function(){
        ...
        this.searchView = $("<div class='acc'><ul></ul></div>")
    ...
    var _emptySearchView = function(){
        if( this.searchView==undefined) return;
    ...
    // ------- Public Method Here -------------
    ControllerMe.prototype.setOption = function(option){
        if ($.isPlainObject(option)) {
    ...
    ControllerMe.prototype.destroy = function(){
        this.searchView.remove();
        this.inputView.unbind('keyup', this._keyup).unbind('keydown', this._keydown).unbind('blur', this._blur);
        delete this.inputView.get(0).ControllerMe;
    };

    ControllerMe.prototype.show = function(){
